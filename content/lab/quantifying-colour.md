...
<mark>All the chromaticitiy points that lie outside the bounded [convex polygon](https://en.wikipedia.org/wiki/Convex_polygon) formed by the cannot be physically recreated, since it would involve creating light with negative intensity. The colours that can be physically created using the primaries is called its [gamut](https://en.wikipedia.org/wiki/Gamut).</mark>
