...
Before quantifying colours, it is important to first define what colours even are. [Wikipedia](https://en.wikipedia.org/wiki/Color) defines it as "The visual perception based on the electromagnetic spectrum". It reveals few important bits of information. A colour is not an objective property of matter, but also depends on the perception of the viewer. What is objective however, is the second part of the informal definition — it is based on the electromagnetic spectrum.
...
![spd for illuminants above; cie xy diagram with faint spectral locus and correlated colour temperature below; points showing chromaticity standard illuminant;](static)
...
{% tangent(summary="White & white", open=true) %}
The 'white' on left is redder while the white on the right is bluer — but the white on the left might appear reddish, while the one on the right might look 'white' white, as the display it is being viewed on is likely to be calibrated to a colour temperature of 6500K. If these exact colours were on a paper (which tend to have lower colour temperatures), the white on left would look 'white', while the white on the right would look more blueish.
{% end %}
...
Dragging the white point to [D65](switch#) or [4500K](switch#) shows how the chromaticity gets transformed to take into account the effects of chromatic adaptation.
{% end %}
