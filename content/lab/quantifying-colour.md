...
Before quantifying colours, it is important to first define what colours even are. [Wikipedia](https://en.wikipedia.org/wiki/Color) defines it as "The visual perception based on the electromagnetic spectrum". It reveals few important bits of information. A colour is not an objective property of matter, but also depends on the perception of the viewer. What is objective however, is the second part of the rather informal definition — it is based on the electromagnetic spectrum.

## Spectral Power Distribution

To start off, we need to know a little about [photons](https://en.wikipedia.org/wiki/Photon). Light is made up of photons. These can thought of tiny abstract particles carrying a specific amount of energy which can be described by their wavelength or freuqency.
...
The full range of photons of different wavelengths forms the [electromagnetic spectrum](https://en.wikipedia.org/wiki/Electromagnetic_spectrum).
...
It is called the 'electromagnetic' spectrum is because a photon is technically a quantum (minimum discrete quantity) of an [electromagnetic field](https://en.wikipedia.org/wiki/Electromagnetic_field). An in-depth explanation of the electromagnetic field is beyond the scope of this article.
{% end %}

Since photons carry energy, any body that radiate photons also radiate energy. The rate of energy transfer via photons can be measured as [power](https://en.wikipedia.org/wiki/Power_(physics)).
...
The set of responses from the cones for non-spectral colours like magenta or gray is different from the set of responses produced by the cones for spectral colours — in fact, a similar response from cones cannot be recreated using light with just one wavelength. The brain perceives these unique set of responses from the cones as a separate colour.
...
{{ loadData(path="/static/media/lab/quantifying-colour/cie-rgb-cmf.txt") }}
...
It is impossible to create light with negative intensity. So it is physically impossible to reproduce certain colours using only three wavelengths of light.
{% end %}
...
{{ loadData(path="/static/media/lab/quantifying-colour/color-space-transform-matrix-photopic-fixed.txt") }}
...
{{ loadData(path="/static/media/lab/quantifying-colour/color-space-transform-matrix-all-fixed.txt") }}
...
In the CIE RGB colour space, the sum of the intensity of its primaries (R+G+B) can act as a crude approximation for luminance, and the ratio among the different intensity of the RGB primaries can serve as an approximation for a colour's chromaticity. Then, the ratios representing r, g, b can be easily calculated by normalizing out the total intensity of the primaries:
...
So, the chromaticity plane contains information about chromaticity, and generally does not contain any information about luminance. So unless luminance is explicitly specified, it is impossible to recreate the corresponding RGB values from just (r,g,b) values.
{% end %}
...
Most displays do not use the CIE RGB primaries though, if any at all. It is economically more efficient to use just three primaries — which are not purely monochromatic. Certain colour spaces are designed keeping such gamut constraints in mind, such as the [sRGB](https://en.wikipedia.org/wiki/SRGB) colour space or the [DCI-P3](https://en.wikipedia.org/wiki/DCI-P3) colour space.
...
A blackbody is an idealized body that does not reflect any light and only emits blackbody radiation. Most bodies in the universe are not technically not blackbodies, but approximating it as one is still useful nonetheless.
{% end %}
...
The [colour temperature](https://en.wikipedia.org/wiki/Color_temperature) is another way to describe *certain* colours and chromaticities — mostly white — and is important since most sources of illumination have a chromaticity close to these values. For example, daylight has a chromaticity similar to a blackbody at temperatures ranging from 5000K to 6500K, and incandescent bulbs emit light having a colour temperature close to 2700K.
...
While in either case the chromaticity does not lie on the planckian locus, it is still close enough, and perceptually similar to be meaningfully attributed to a colour temperature. These chromaticity values can be assigned to a [correlated colour temperature](https://en.wikipedia.org/wiki/Correlated_color_temperature) based on how closely it resembles the chromaticity of an ideal blackbody at that temperature.

The correlated colour temperature can also be mapped alongside the planckian locus:
...
The lines intersecting the planckian locus represent correlated colour temperatures. Two points on a correlated colour temperture line can be considered to have the same correlated correlated colour temperature.
{% end %}

The correlated colour temperature can describe non-ideal blackbodies and other sources of white light — but since they there are multiple chromaticity values that correspond to a single correlated colour temperature, it is not a definitive way to describe white light.

To represent white light from non-idealized blackbody radiation without ambiguity, [standard illuminants](https://en.wikipedia.org/wiki/Standard_illuminant) have been defined — theoretical souces of light with a definite spectral power distribution. For example, [D65](https://en.wikipedia.org/wiki/Standard_illuminant#Illuminant_series_D) is a standard illuminant with an exact chromaticity point that represents average daylight.

{{ loadData(path="/static/media/lab/quantifying-colour/xy-plane-cie-planckian-locus-standard-illuminants-spd.txt") }}
<!-- ![spd for illuminants above; cie xy diagram with faint planckian locus and correlated colour temperature below; points showing chromaticity standard illuminant;](static) -->

Having a standard way to define illumination sources is necessary because of [chromatic adaptation](https://en.wikipedia.org/wiki/Chromatic_adaptation). Human colour perception adjusts for variations in illumination to preserve colours of objects. Two colours might appear white despite having different 'true' colours depending on the environment and viewing medium.

{{ loadData(path="/static/media/lab/quantifying-colour/different-white-points.txt") }}
<!-- ![comparison reddish white vs bluish white — http://brainden.com/images/color-cube-big.jpg, or image similar to white-golden or blue-black dress image](static) -->
