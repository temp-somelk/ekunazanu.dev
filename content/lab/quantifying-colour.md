...
It was decided that a new different colour space that could map all perceivable colours to non-negative values was more preferable. But instead of conducting more experiments to construct a new colour space, the existing CIE RGB space can also be transformed using simple linear transformations to a. The transformation of the three dimensional colour space can then be defined by a 3x3 matrix:
