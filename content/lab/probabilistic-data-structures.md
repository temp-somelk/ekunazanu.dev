...
A hash function is any mathematical function that maps an input to an output. In practice, the output of hash functions have certain properties. These properties of most hash functions are at the core of these data structures, so it needs to be discussed first. Most hashing functions produce output that are:
...
These properties are immensely useful and form the basis of most of the different types of probabilistic data structures.
...
Rather than outright specifying how bloom filters work, in this case it's much more fruitful to discuss possible solutions to tackle the problem. It'll help build a natural intuition as to why bloom filters are a better approach (when accuracy is not a big priority). So, how do we know if something exists in a set using as little space as possible? How do we store information using less information?
...
Since hash functions produces fixed length output, this is arguably better than storing long elements and somewhat better than storing compressed elements. But using a simple but rather clever trick, it can be made even more efficient.

#### Bit Arrays

Because most hashing functions always produce a fixed length number for any input, it can be exploited to exponentially decrease the amount of storage needed to store the hashes.
